...
async def process_description(message: Message, state: FSMContext):
    ...
    try:
        if "expense_id" in data:
            ...
            response = api_client.update_expense(expense_id, {
                "category": data["title"],
            ...
            response = api_client.create_expense({
                "category": data['title'],
                "amount_uah": data['amount'],
                "date": data['date'],
                "description": data['description'],
        ...
        if response.status_code == 200:
            await message.answer("✅ Операція успішна!", reply_markup=main_menu())
        else:
            await message.answer(f"⚠️ Помилка: {response.text}", reply_markup=main_menu())
...
async def handle_edit_expense(callback: CallbackQuery, state: FSMContext):
    response = api_client.get_expenses()
...
async def process_end_date(message: Message, state: FSMContext):
    try:
        ...
        response = api_client.get_report(start_date, end_date_str)
